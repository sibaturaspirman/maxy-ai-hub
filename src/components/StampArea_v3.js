...
export default function StampArea({ profile, booth, mallData }) {
  ...
  // Threshold untuk masing-masing jarak (sesuaikan dengan kebutuhan)
  const thresholds = {
    upperToLowerRight: { min: 160, max: 265 },
    lowerLeftToLowerRight: { min: 105, max: 195 },
    upperToLowerLeft: { min: 100, max: 290 }
  };
  ...
  const [debugInfo, setDebugInfo] = useState({
    ...
    thresholds: {
      upperToLowerRight: { min: 170, max: 198 },
      lowerLeftToLowerRight: { min: 160, max: 188 },
      upperToLowerLeft: { min: 110, max: 140 }
    },
    isValid: {
  ...
  const handleTouchStart = async (e) => {
    ...
    if (points.length === 3) {
      ...
      if (upperRight && lowerLeft && lowerRight) {
        ...
        const isValidUpperToLowerRight = distUpperToLowerRight >= thresholds.upperToLowerRight.min && 
          distUpperToLowerRight <= thresholds.upperToLowerRight.max;
        const isValidLowerLeftToLowerRight = distLowerLeftToLowerRight >= thresholds.lowerLeftToLowerRight.min && 
          distLowerLeftToLowerRight <= thresholds.lowerLeftToLowerRight.max;
        const isValidUpperToLowerLeft = distUpperToLowerLeft >= thresholds.upperToLowerLeft.min && 
          distUpperToLowerLeft <= thresholds.upperToLowerLeft.max;
        ...
        // Gambar 3 titik dengan debug
        // drawThreePoints(ctx, canvas, upperRight, lowerLeft, lowerRight, {
        //   upperToLowerRight: distUpperToLowerRight,
        //   lowerLeftToLowerRight: distLowerLeftToLowerRight,
        //   upperToLowerLeft: distUpperToLowerLeft
        // });

        // Cek apakah semua jarak valid
        const allDistancesValid = isValidUpperToLowerRight && isValidLowerLeftToLowerRight && isValidUpperToLowerLeft;

        if (allDistancesValid) {
          // Semua jarak cocok, langsung proses stamp
          await handleStamp();
        } else {
          // Reset statusStamp jika belum valid
  ...
  const checkThreePointPattern = (points) => {
    ...
    // Gunakan thresholds yang sudah didefinisikan di atas

    const isValidDistance = 
      distUpperToLowerRight >= thresholds.upperToLowerRight.min && 
      distUpperToLowerRight <= thresholds.upperToLowerRight.max &&
      distLowerLeftToLowerRight >= thresholds.lowerLeftToLowerRight.min && 
      distLowerLeftToLowerRight <= thresholds.lowerLeftToLowerRight.max &&
      distUpperToLowerLeft >= thresholds.upperToLowerLeft.min && 
      distUpperToLowerLeft <= thresholds.upperToLowerLeft.max;
    ...
    return isUpperRightValid && isLowerRightValid && isLowerLeftValid && isValidDistance;
  };
        ...
        <div className="hiddenx absolute z-[50] mt-[-5rem] pointer-events-none">
          ...
          <div className="mt-4 flex justify-center gap-3" style={{ fontFamily: 'monospace', fontSize: '11px' }}>
            <span style={{ color: debugInfo.isValid?.upperToLowerRight ? '#3B82F6' : '#9CA3AF' }}>
              UR→LR: {Math.round(debugInfo.distances.upperToLowerRight)}
            </span>
            <span style={{ color: debugInfo.isValid?.lowerLeftToLowerRight ? '#3B82F6' : '#9CA3AF' }}>
              LL→LR: {Math.round(debugInfo.distances.lowerLeftToLowerRight)}
            </span>
            <span style={{ color: debugInfo.isValid?.upperToLowerLeft ? '#3B82F6' : '#9CA3AF' }}>
              UR→LL: {Math.round(debugInfo.distances.upperToLowerLeft)}
            </span>
          </div>
            ...
            <div className="mb-2">
              <div className="flex justify-between items-center">
                ...
                <span className={`font-bold ${debugInfo.isValid?.upperToLowerRight ? 'text-green-400' : 'text-red-400'}`}>
                  {Math.round(debugInfo.distances.upperToLowerRight)}px
                </span>
              </div>
              <div className="text-xs text-gray-400 ml-2">
                Threshold: {debugInfo.thresholds?.upperToLowerRight.min} - {debugInfo.thresholds?.upperToLowerRight.max}px
            ...
            <div className="mb-2">
              <div className="flex justify-between items-center">
                ...
                <span className={`font-bold ${debugInfo.isValid?.lowerLeftToLowerRight ? 'text-green-400' : 'text-red-400'}`}>
                  {Math.round(debugInfo.distances.lowerLeftToLowerRight)}px
                </span>
              </div>
              <div className="text-xs text-gray-400 ml-2">
                Threshold: {debugInfo.thresholds?.lowerLeftToLowerRight.min} - {debugInfo.thresholds?.lowerLeftToLowerRight.max}px
            ...
            <div className="mb-2">
              <div className="flex justify-between items-center">
                ...
                <span className={`font-bold ${debugInfo.isValid?.upperToLowerLeft ? 'text-green-400' : 'text-red-400'}`}>
                  {Math.round(debugInfo.distances.upperToLowerLeft)}px
                </span>
              </div>
              <div className="text-xs text-gray-400 ml-2">
                Threshold: {debugInfo.thresholds?.upperToLowerLeft.min} - {debugInfo.thresholds?.upperToLowerLeft.max}px
